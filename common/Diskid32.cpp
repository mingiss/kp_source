...
// 2003.10.30 adopted to run in kp environment by minde4s@one.lt
...
#define  CAP_IDE_EXECUTE_SMART_FUNCTION  4  // SMART commannds supported
   ...
   //  IDE registers
#if ((Compiler != Watcom) || (__WATCOMC__ < 1202)) && ((Compiler != Mingw) || (MingwVer < 471))
      ...
      if (hPhysicalDriveIOCTL != INVALID_HANDLE_VALUE)
      {
         ...
         // Now, get the ID sector for all IDE devices in the system.
            ...
            if ( DoIDENTIFY (hPhysicalDriveIOCTL,
                       &scip,
                       (PSENDCMDOUTPARAMS)IdOutCmd, // (PSENDCMDOUTPARAMS)&IdOutCmd,
                       (BYTE) bIDCmd,
                       (BYTE) drive,
                       &cbBytesReturned))
            {
